...
// ─── Icons (module scope — stable identity, no closure over render state) ──────
...
const BADGE = "absolute z-2 top-[-6px] right-[-8px] bg-[#370E4D] text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center"
...
const DropNavbar = () => {
  ...
  // Mirrors the account dot above: saved items hydrate from localStorage after mount.
  ...
  return (
    <header className="sm:px-10 px-4 py-2 fixed z-50 w-full bg-black border-b border-white/10 text-white">
      <nav className="flex justify-between items-center max-w-screen xl:mx-auto z-50">
        ...
        <div className="w-1/4 flex gap-3 pointer-coarse:gap-2 md:gap-8">
          ...
          <div className="sm:hidden">
            <button
              ...
              className="flex items-center gap-1.5 border-none bg-transparent cursor-pointer text-white hover:text-white/60 transition duration-200"
            >
        ...
        <Link href="/" className="w-1/2 items-center flex justify-center">
          <h1 className="text-3xl sm:text-4xl tracking-wide text-white">Enunas</h1>
        </Link>
        ...
        <div className="flex items-center gap-3 pointer-coarse:gap-2 md:gap-8 text-base w-1/4 justify-end">
          ...
          <div className="relative">
            <Link href="/account" aria-label="Mein Konto">
              <AccountIconSvg className="w-5 h-5 text-white hover:text-white/60 transition-colors duration-200" />
            </Link>
            {isAuthenticated && (
          ...
          <div className="hidden sm:block relative">
            <Link
              ...
              aria-label={savedCount > 0 ? `Wunschliste mit ${savedCount} ${savedCount === 1 ? 'Artikel' : 'Artikeln'}` : 'Wunschliste'}
            >
              <HeartIconSvg className="w-5 h-5 text-white hover:text-white/60 transition-colors duration-200" />
            </Link>
            {savedCount > 0 && (
              <span aria-hidden="true" className="absolute top-[-4px] right-[-4px] w-2 h-2 rounded-full bg-enunas-purple" />
            )}
          </div>

          <div>
            <Link href="/cart" className="relative cursor-pointer" aria-label={`Warenkorb mit ${itemCount} Artikeln`}>
              <CartIconSvg className="w-5 h-5 text-white hover:text-white/60 transition-colors duration-200" />
              {itemCount > 0 && (
