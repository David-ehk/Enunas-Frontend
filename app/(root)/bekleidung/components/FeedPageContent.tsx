...
export default function FeedPageContent({ basePath, HeroComponent }: Props) {
  ...
  const [gender, setGender] = useState<('damen' | 'herren')[]>(() => {
    ...
    return g === 'damen' ? ['damen'] : g === 'herren' ? ['herren'] : []
  })
  ...
  return (
    <div style={{ background: '#fff', fontFamily: "'League Spartan', sans-serif", color: '#0A0A0A', minHeight: '100vh' }}>
      ...
      <HeroComponent count={visibleProducts.length} loading={loading} />

      {/* Sticky category strip */}
      <div style={{ position: 'sticky', top: navH, zIndex: 20, background: '#fff', borderBottom: '1px solid #E8E8E8' }}>
        <CategoryNavigation basePath={basePath} />
      </div>
      ...
      <div ref={filterBarRef} style={{
        ...
        <div style={{ display: 'flex', gap: 0, alignItems: 'center' }}>
          {(['Damen', 'Herren'] as const).map((label) => {
            ...
            return (
              <button key={id} onClick={() => toggleGender(id)} style={{
                position: 'relative', background: 'none', border: 'none', cursor: 'pointer',
                padding: '2px 16px 5px 0', fontFamily: 'inherit', fontSize: 10,
                ...
                {label}
                <span style={{
                  position: 'absolute', left: 0, right: 16, bottom: 0,
                  height: 2, background: accentColor, transformOrigin: 'left',
        ...
        <div style={{ display: 'flex', gap: isMobile ? 16 : 24, alignItems: 'center' }}>
          <button onClick={() => setFilterOpen(true)} style={{
            ...
            color: '#0A0A0A', fontFamily: 'inherit', padding: 0, position: 'relative',
          }}>
            Filter
            {activeFilterCount > 0 && (
              <span style={{
      ...
      {/* Product grid */}
      <section style={{ maxWidth: 1800, margin: '0 auto', padding: isMobile ? '24px 16px 72px' : '40px 48px 96px' }}>
        {loading ? (
          <div style={{
            display: 'grid',
