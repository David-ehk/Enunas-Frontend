...
interface CompleteTheLookProps {
  /** Up to 4 outfit pieces. Layout adapts: 1 → single big card, 2 → stacked, 4 → 2x2 */
  items: RecItem[];
  /** Optional hero image of the full outfit (Ami Paris style). Falls back to figure silhouette. */
  heroImage?: string;
  /** Link to all outfit suggestions */
  allHref?: string;
}

/**
 * "Vervollständige den Look" — Ami Paris-style:
 * left = big outfit hero, right = adaptive grid of the individual pieces.
 */
  ...
  return (
    <section className="px-16 py-6 max-w-[1800px] mx-auto">
      ...
      <div className="grid grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)] gap-4">
        ...
        {/* Adaptive grid of pieces */}
        <div className={`grid ${gridCols} gap-3`} data-count={count}>
          {visible.map((item) => (
