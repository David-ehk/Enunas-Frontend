...
const Navbar = () => {
  ...
  // Saved items live in localStorage and load in an effect, so this is empty on the first paint
  // and fills in after mount — the same timing as isAuthenticated, so both dots appear together.
      ...
      <nav className="flex justify-between items-center max-w-screen xl:mx-auto z-50">
        ...
        <div className="flex items-center gap-3 pointer-coarse:gap-2 md:gap-8 w-1/4 justify-end">
          ...
          <div className="relative">
            <Link
              href="/saved-lists"
              className={`cursor-pointer ${TOUCH_HIT}`}
